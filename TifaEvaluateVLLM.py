...
def vllm_evaluation():
    ...
    eval_dataset = pd.read_csv("./data/tifa_eval_dataset.csv")
    ...
    @torch.inference_mode
    def get_sample(row):
        user_content_prompt = f"Generate SVG image from description: <{row.caption[:-1]}>."
        return [
            {"role": "system", "content": optional_prompt},
            {"role": "user", "content": user_content_prompt}
            ]
    ...
    for idx in tqdm(range(10), desc="Генерация SVG"):
